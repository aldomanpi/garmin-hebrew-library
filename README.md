...
│       ├── hebrew.fnt           # Pre-built BMFont descriptor (text)
│       └── hebrew.png           # Glyph sprite sheet  ← binary, see setup
└── tools/
...
### 1. Get the binary font assets

Two binary files are required that cannot be stored as plain text:

| File | Size | How to get it |
|------|------|---------------|
| `resources/fonts/Alef-Garmin.ttf` | ~84 KB | Copy from `watch-siddur/resources/fonts/` or supply any Hebrew TTF |
| `resources/fonts/hebrew.png` | ~62 KB | Generated by `gen_font.py` (see step 3) |

```bash
# If you have watch-siddur cloned alongside this repo:
cp ../watch-siddur/resources/fonts/Alef-Garmin.ttf resources/fonts/
cp ../watch-siddur/resources/fonts/hebrew.png       resources/fonts/
```

If you copied `hebrew.png` from watch-siddur you can skip step 3 as long
as your app text is a subset of BirchatHamazon.  Otherwise run step 3.

### 2. Install the Python tool dependencies

```bash
pip install Pillow
```

### 3. Generate the font for your Hebrew text
...
The tool overwrites `resources/fonts/hebrew.fnt`, `resources/fonts/hebrew.png`,
and each file listed in `--mc-files` with a PUA-normalised version.
The original Unicode source (with nikkud) is preserved in git history.
