...
// Smooth-scrolling RTL Hebrew text viewer for Garmin ConnectIQ.
//
// Quick start:
//   var view = new HebrewTextView("Title", [myPuaEncodedText]);
//   WatchUi.pushView(view, new HebrewTextDelegate(view), WatchUi.SLIDE_LEFT);
//
// Text must be PUA-encoded before embedding in string literals.
// Run tools/gen_font.py on your source files to produce both the
// PUA-normalised .mc files and the matching hebrew.fnt / hebrew.png assets.
//
// Text block format (each element of the lines array):
//   "\n\n"  paragraph break (blank line between paragraphs)
//   "\n"    soft break (treated as a space by the word-wrapper)
//   "|..."  section header (rendered in COLOR_LT_GRAY)
class HebrewTextView extends WatchUi.View {
    ...
    // title: shown in a top bar; pass "" to hide the bar entirely.
    ...
    // Replace the built-in hebrewFont resource with a custom FontDefinition.
    // Call before the view is first drawn; triggers a layout rebuild.
    ...
    // Scroll one line toward the end of the text.  Returns false when already
    // at the last line (caller may then pop the view).
    function scrollDown() as Boolean {
        if (mFontH == 0) { return false; }
        var maxScroll = _maxScrollPx();
        if (mScrollTarget < maxScroll) {
            mScrollTarget += mFontH;
            if (mScrollTarget > maxScroll) { mScrollTarget = maxScroll; }
            _startScrollTimer();
            return true;
        }
        return false;
    ...
    // Scroll one line toward the beginning.  Returns false when already at top.
    ...
    // Called every 16 ms by the scroll timer.  Handles two modes:
    //   momentum — friction-coast after a touch release (0.92 decay/tick)
    ...
    public function onScrollTick() as Void {
        if (mMomentum) {
            ...
            var step = mVelocity.toNumber();
            var newScroll = mScrollPx + step;
            var mx = _maxScrollPx();
    ...
    private function _snapToLine() as Void {
        ...
        if (rounded != mScrollPx) {
            mScrollTarget = rounded;
            _startScrollTimer();
        }
    }
    ...
    // already in motion.  We absorb it as the new reference rather than
    // computing a delta, so the first visible frame is always incremental.
    //
    // Momentum: velocity is a 60/40 EMA of per-frame deltas.  On release, if
    // speed exceeds 1.5 px/frame the timer coasts with 0.92 friction per tick.
    ...
    // Call this when a BehaviorDelegate swipe event interrupts the drag
    // sequence so momentum fires correctly for fast flicks.
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        if (!mLayoutDone) {
            ...
            for (var p = 0; p < mLines.size(); p++) {
                ...
                var pl = HebrewText.layoutLines(dc, mLines[p],
        ...
        var titleH  = mTitle.length() > 0 ? dc.getFontHeight(Graphics.FONT_XTINY) + 6 : 0;
        ...
        // Redraw title bar on top so text scrolling under it is masked.
        if (mTitle.length() > 0) {
    ...
    private function _pickFont() as Graphics.FontDefinition {
        ...
        if (f == null) {
            f = Graphics.FONT_MEDIUM as Graphics.FontDefinition;
        }
